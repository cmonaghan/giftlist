...
var prepareAmazonDataForParse = function(amazonResponse) {
  var dataForParse = [];
...
var saveDataToParse = function(items) {
  // ****** This loops through and saves each item to parse ******* //
  for (var i = 0; i < items.length; i++) {
    app.insert('Items', items[i], function(err, response) {
      if (err) {
...
prodAdv.call("ItemSearch", options, function(err, response) {
  if (err) {
    console.log(err);
  } else {
